...
We encountered inference methods for a single proportion in the Martian alphabet example of Chapter \@ref(foundations-randomization),the  Medical consultant case study of Chapter \@ref(foundations-bootstrapping) exploring point estimates, confidence intervals, and hypothesis tests.
...
In this section, we will demonstrate the steps of a hypothesis test in the context of the medical consultant case study from Section \@ref(case-study-med-consult).
A medical consultant tried
...
No. To assess the hypotheses, we need to see a distribution of many simulated sample proportions ($\hat{p}_{sim}$), not just a *single* draw from this sampling distribution.
:::
