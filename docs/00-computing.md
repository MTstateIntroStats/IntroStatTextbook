...
We recommend accessing this program through the MSU RStudio server: [rstudio.math.montana.edu/](https://rstudio.math.montana.edu/). Please reference the [Statistical computing](#stat-computing) section of the Welcome chapter for other options for accessing RStudio and for instructions on how to download and install `R` and RStudio on your own computer.
