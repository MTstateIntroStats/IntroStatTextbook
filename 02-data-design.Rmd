...
```{r include=FALSE}
terms_chp_2 <- c("population", "sample")
```
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2, "parameter", "statistic", "sampling variability")
```
...
```{r mn-winter, out.width="35%", fig.cap='In February 2010, some media pundits cited one large snow storm as evidence against global warming. As comedian Jon Stewart pointed out, "It is one storm, in one region, of one country."'}
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2, "sampling bias", "simple random sample", "selection bias", "non-response bias", "response bias")
```
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2, "non-response rate", "representative", "non-response bias", "sampling frame")
```
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2, "replicate")
```
