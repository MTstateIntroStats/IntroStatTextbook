...
stent %>%
  ...
  scale_fill_brewer() + theme_classic() +
  labs(y = "proportion")
```
...
Reminder: for these Guided Practice questions, you can check your answer in the footnote.^[The loan's grade is A, and the borrower rents their residence.]
```
...
```{block2, type="data", echo=TRUE}
These data can be found in the [openintro](http://openintrostat.github.io/openintro) package: [`county`](http://openintrostat.github.io/openintro/reference/county.html).
```
...
Replace MLB example with something else below
```
...
```{block2, type="todo", echo=TRUE}
Are the "four" principles the same as the enumerated list above?
```
