...
12.  Fill in the blanks with the appropriate variables and values to plot a segmented bar plot of injury by helmet use.
...
- $z = \frac{\text{point estimate} - \text{null value}}{SE}$, where the standard error is calculated using the pooled proportion of successes.

- $SE(\hat{p}_1-\hat{p}_2)=\sqrt{\left(\frac{\hat{p}_{pool}(1-\hat{p}_{pool})}{n_1-1}+\frac{\hat{p}_{pool}(1-\hat{p}_{pool})}{{n_2-1}}\right)}$, where $\hat{p}_{pool} = \frac{\text{number of "successes"}}{\text{number of cases}} = \frac{\hat{p}_1 n_1+\hat{p}_2 n_1}{n_1+n_2}$
...
To find a confidence interval for the difference in proportions we will add and subtract the margin of error from the point estimate to find the two endpoints.
   ...
   $$\hat{p}_1-\hat{p}_2\pm SE(\hat{p}_1-\hat{p}_2), \text{where}$$
