...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2,"boxplot","five number summary")
```

```{r boxPlotLayoutNumVar, echo=FALSE, out.width="75%", fig.cap="A vertical dot plot next to a labeled box plot for the number of characters in 50 emails. The median (6.89), splits the data into the bottom 50% and the top 50%, marked in the dot plot by horizontal dashes and open circles, respectively."}
include_graphics("02/figures/boxPlotLayoutNumVar/boxPlotLayoutNumVar.png")
```
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2,"outlier")
```
...
`r colorize("-one cat variable plot
...
```{r include=FALSE}
terms_chp_2 <- c(terms_chp_2, "contingency table", "two-way table",
...
dimnames(tab) <- list(Type = c("spam", "not spam","Total"),
                   Number = c("none", "small","big","Total"))

kable(tab, caption = "Contingency table of `Type` and `Number` variables.")
```
