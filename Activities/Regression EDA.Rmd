...
par(mfrow=c(1,2))
scatterplot(revenue~budget, data=moviesa,pch=20,main="Revenue vs. Budget", smooth=F)
```
4. Assess the four characteristics of the scatterplot for this relationship.
...
```{r}
moviesb <- moviesa[,c("budget", "revenue", "duration", "imdb_score", "movie_facebook_likes")]
...
```{r}
favstats(moviesa$budget)
favstats(moviesa$revenue)
```
...
```{r}
revenueLM <- lm(revenue~budget,data=moviesa)
summary(revenueLM)
```
...
15. Using the least squares line from Question 14, predict the revenue for a movie with a budget of 4,946,250.
...
16.  The movie, Independence Day: Resurgence, had a budget of 165,000,000 and revenue of 102,315,545.  Find the residual for this movie.
...
scatterplot(revenue~budget|content_rating, data = moviesa, smooth=F)
```
