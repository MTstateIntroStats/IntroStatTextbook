...
Since the formulas for these standard deviations depend on unknown parameters, we substitute our "best guess" for $\pi$ in the formulas, either from a hypothesis or from a point estimate. The resulting _estimated_ standard deviation is called the **standard error** of the statistic.
```
...
include_graphics("05/figures/soe_gradient.png")
```
...
```{block2, type="onebox", echo=TRUE}
**Bootstrapping^[If you're curious where the term "bootstrapping" comes from, it comes from the phrase "lift yourself up by your own bootstraps." Lifting yourself up by your own bootstraps is analogous to creating more samples from the single original sample.] from one sample**.
...
\]
where $p_0$ is the null value. The denominator, $\sqrt{\frac{p_0(1-p_0)}{n}}$, is called the **null standard error** of the sample proportion.
```
...
```{r paydayCC-stdnorm-pvalue, fig.cap="Approximate sampling distribution of $Z$ across all possible samples assuming $\\pi = 0.50$. The shaded area represents the p-value corresponding to an observed standardized statistic of 0.57. Compare to Figure @\ref(fig:paydatCC-norm-pvalue)", warning=FALSE, fig.width=10}
...
For hypothesis testing, we often use $p_0$ as the best guess of $\pi$, as seen in Section \@ref(theory-prop).  For confidence intervals, we typically use $\hat{p}$ as the best guess of $\pi$.
```
...
-->
```{block2, type="tipbox", echo=TRUE}
The success-failure condition listed above is only necessary for the sampling distribution of $\hat{p}_1 - \hat{p}_2$ to be approximately normal. The mean of the sampling distribution of $\hat{p}_1 - \hat{p}_2$ is $\pi_1 - \pi_2$, and the standard deviation is $\sqrt{\frac{\pi_1(1-\pi_1)}{n_1}+\frac{\pi_1(1-\pi_1)}{n_1}}$, regardless of the two sample sizes.
```
...
```{r buildOpportunityData, echo = FALSE}
buy <- c(rep("buy DVD", 97), rep("not buy DVD", 53))
group <- rep.int(c("control", "treatment", "control", "treatment"), times = c(56, 41, 19, 34))

opportunity <- data.frame(buy, group)
```

For comparisons of two proportions, we get a two-way table.  Suppose in this case we have the data from the opportunity cost study in a data frame called `opportunity`, with variables `buy` giving the student's decision and `group` indicating whether they were in the treatment or control group.  The `summary()` function can help us see what variables are in our dataset and the values they take on:
```{r showOpportunityData, echo = TRUE}
summary(opportunity)
```
...
```{r crosstabOpportunity, echo = TRUE}
table(opportunity$buy, opportunity$group)
```
...
```{r storeCrostab, echo = TRUE}
data_tbl <- table(opportunity$buy, opportunity$group)
```

To get column percentages, we use the `prop.table()` function:
```{r columnPctsCrosstab, echo = TRUE}
...
```{r showOnePropSimCall, eval = FALSE, echo = TRUE}
one_proportion_test(
...
For inference about the difference between two proportions, we use the `two_proportion_test()` and `two_proportion_bootstrap_CI()` functions.  These functions assume you have a dataset with the group and outcome included as variables. Using the opportunity cost study, a call to `two_proportion_test()` would look like this:
```{r twoPropSimCall, eval = FALSE, echo = TRUE}
two_proportion_test(
  formula = buy ~ group,  #always do outcome ~ grouping factor
  data = opportunity,   #Name of the data set
  first_in_subtraction = "treatment", #Value of the group variable that is first in order of subtraction
  response_value_numerator = "not buy DVD",  #Value of outcome that is a "success"
  number_repetitions = 1000,
  as_extreme_as = 0.2,  #Observed statistic
  direction = "greater"  #direction of the alternative hypothesis
)
```
...
This produces the distribution of bootstrap values with the bounds of the confidence interval marked, and the value included in the caption.  Here, we are 95% confident that the true proportion of students who receive the treatment that do not buy the DVD than students is between 0.04 and 0.347 higher than the proportion who do not buy the DVD under the control condition.
...
```{r onePropTheoryCall, echo = TRUE, eval = FALSE}
...
When comparing two proportions, we use the same function, but the inputs are a little different.  First, we need to create our table of outcomes, which then becomes our primary input to `prop.test()`.  As an example, we will again use the opportunity cost study.  There is one important step to take before creating our table: `R` will always put the values of variables in alphabetical order when building tables, unless told otherwise.
```{r showTableOrder, echo = TRUE}
table(opportunity$buy, opportunity$group)
```
...
```{r relevelTwoProp, echo = TRUE}
#Switch order of subtraction:
opportunity$group <- relevel(opportunity$group, ref = "treatment")
table(opportunity$buy, opportunity$group)

#Switch "success":
opportunity$buy <- relevel(opportunity$buy, ref = "not buy DVD")
table(opportunity$buy, opportunity$group)


```

```{r twoPropTheoryCall, echo = TRUE, eval = FALSE}
data_tbl <- table(opportunity$buy, opportunity$group)
...
```
When comparing two proportions, we don't need the `n` or `p` arguments.  We do still have to re-run as "two.sided" to get the correct confidence interval. The observed proportions in each group are given under sample estimates: `R` will always take (prop 1 - prop 2), so our alternative of "greater" means (prop 1 > prop 2). If the observed proportions don't match your calculations of the proportion of successes or are in the wrong order of subtraction, go back and `relevel()` to correct the problem.

```{r twoPropTheoryInf, echo = TRUE}
data_tbl <- table(opportunity$buy, opportunity$group)
...
Here, we obtain a p-value of 0.0052, strong evidence against the null hypothesis of no difference between the two groups.  
```{r twoPropTheoryCI, echo = TRUE}
prop.test(x = data_tbl,
...
Our 95% confidence interval for the true difference in the proportion of students who do not buy the DVD is between 0.057 and 0.381 greater for students in the treatment group.
